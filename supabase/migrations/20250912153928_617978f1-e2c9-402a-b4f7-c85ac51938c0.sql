-- Create activity_logs table for auditing recent activities
...
-- Foreign key to profiles so we can display who made the change
...
-- Allow medical staff to read logs
CREATE POLICY IF NOT EXISTS "Medical staff can view activity logs"
ON public.activity_logs FOR SELECT
USING (public.is_medical_staff());

-- Allow inserts via triggers (executed under end-user role) for all authenticated users
